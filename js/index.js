...
function addSecondary(sec, div) {
    ...
    switch (sec){
        case "ASSASSINATION":
           card.innerHTML = `<h3>ASSASSINATION</h3>Score 3 victory points at the end of the battle for each enemy CHARACTER model that is destroyed. If the enemy WARLORD was destroyed during the battle, gain 1 additional victory point.`
           break;
        case "BRING IT DOWN":
            card.innerHTML = `<h3>BRING IT DOWN</h3>Score 1 victory point at the end of the battle for each enemy MONSTER or VEHICLE model with a Wounds characteristic of 9 or less that is destroyed, 2 victory points for each enemy MONSTER or VEHICLE model with a Wounds characteristic of between 10-14 that is destroyed, and 3 victory points for each enemy MONSTER or VEHICLE model with a Wounds characteristic of 15 or more that is destroyed.`
            break;
        case "TITAN HUNTER":
            card.innerHTML = `<h3>TITAN HUNTER</h3>Score 4 victory points at the end of the battle if one enemy TITANIC model is destroyed, 9 victory points if two enemy TITANIC models are destroyed, or 15 victory points if three or more enemy TITANIC models are destroyed.`
            break;
        case "FOR THE EMPEROR! (Astra Militarum)":
            card.innerHTML = `<h3>FOR THE EMPEROR! (Astra Militarum)</h3>Score 1 victory point at the end of the battle round for each enemy unit that was destroyed during that battle round by an ASTRA MILITARUM unit from your army (to a maximum of 3 victory points per battle round).`
            break;
        case "NO PRISONERS":
